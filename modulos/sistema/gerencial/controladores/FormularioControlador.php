<?php
    ...
    try{
        if($strAcao == "Consultar"){             
            $arrObjs = FachadaGerencial::getInstance()->consultarFormulario($_POST);
            ...
            if(FachadaGerencial::getInstance()->salvarFormulario($_POST)){                
                $arrStrJson["sucesso"]  = "true";
                $arrStrJson["mensagem"] = MensagemHelper::getInstance()->getOperacaoRealizadaComSucesso();
            }
        }elseif($strAcao == "Excluir"){                
            if(FachadaGerencial::getInstance()->excluirFormulario($_POST)){                
                $arrStrJson["sucesso"]  = "true";
                $arrStrJson["mensagem"] = MensagemHelper::getInstance()->getOperacaoRealizadaComSucesso();
            }
        }
