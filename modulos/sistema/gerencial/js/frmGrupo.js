...
var controlador  = "../../sistema/gerencial/controladores/GrupoControlador.php";
var dg           = $('#grid');
...
function initGRID(){ 
    dg.datagrid({
                        ...
                        $.post(controlador, {
                            ...
                            function(data){                                
                                if(data.sucesso == "true"){                                    
                                    $("#txtDescricao").val(data.rows[0].GRU_Descricao);
                                    
                                    if(data.rows[0].GRU_Status == "I"){ 
        ...
        ,mapper:[{
            name: 'GRU_ID', title: 'Cód.', width: 60, align: 'left'
        },{
            name: 'GRU_Descricao', title: 'Descrição', width: 880, align: 'left'
        },{
            name: 'GRU_Status', title: 'Ativo', width: 60, align: 'center', render: function(d) {
                var status = 'SIM';
...
function salvar(){
    ...
    if($.trim($('#txtDescricao').val()) == ""){        
        ...
        $("#dialog-atencao").html("Por favor, informe a descrição do grupo.");
        $("#dialog-atencao").dialog("open");
        return;
    }
