...
var controlador  = "../../livroRegistro/livro-auxiliar/controladores/FolhaAuxiliarControlador.php";
var livroControlador  = "../../livroRegistro/livro-auxiliar/controladores/LivroAuxiliarControlador.php";
var dg           = $('#grid');
...
function consultar(){ 
    ...
    if(data.sucesso == "false"){
        ...
        var params = {
            ...
            LIA_ID: selPesquisaLivro,
            FAU_NumeroFolha: txtPesquisaNumeroFolha,
            FAU_DataFolha: txtPesquisaData,
...
function initGRID(){ 
    dg.datagrid({
                        ...
                        $.post(controlador, {
                            ...
                            function(data){                                
                                ...
                                if(data.sucesso == "true"){
                                    $("#hddID").val(data.rows[0].FAU_ID);

                                    $("#selLivro").val(data.rows[0].LIA_ID);
                                    $("#selLivro").trigger('chosen:updated');

                                    $("#txtNumeroFolha").val(data.rows[0].FAU_NumeroFolha);         
                                    $("#txtData").val(data.rows[0].FAU_DataFolha);         
        ...
        ,mapper:[{
            name: 'FAU_ID', title: 'Cód.', width: 60, align: 'center'        
        },{
            name: 'LIA_NumeroLivro', title: 'Livro', width: 100, align: 'left'                        
        },{
            name: 'FAU_NumeroFolha', title: 'Numero Folha', width: 300, align: 'left'                
        },{
            name: 'totalLinhas', title: 'Quantidade de Linhas', width: 150, align: 'left'                            
        },{
            name: 'FAU_DataFolha', title: 'Data', width: 80, align: 'left'                
        },{
...
function consultarPermissaoSalvarLivro(idLivro){    
    ...
    $.ajax({
        ...
        cache: false,
        async:false,    
        data: {LIA_ID: idLivro, ACO_Descricao: "GetPermissaoAddFolhaLivro"}
    }).done(function( data ) {  
        if(data.sucesso == "true"){                                       
            retorno = true;                
        }else{            
