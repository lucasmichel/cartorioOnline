<?php    
    ...
    try{
        if($strAcao == "Consultar"){            
            $arrObjs = FachadaLivroPrevio::getInstance()->consultarLinhaPrevio($_POST);
            if($arrObjs != null){
            ...
            $_POST["LPR_DataHoraCadastro"] = date("Y-m-d H:i:s");
            $_POST["LPR_DataHoraAlteracao"] = date("Y-m-d H:i:s");
            
            if(FachadaLivroPrevio::getInstance()->salvarLinhaPrevio($_POST)){                
                $arrStrJson["sucesso"]  = "true";
                $arrStrJson["mensagem"] = MensagemHelper::getInstance()->getOperacaoRealizadaComSucesso();
            }
        }elseif($strAcao == "Excluir"){
            if(FachadaLivroPrevio::getInstance()->excluirLinhaPrevio($_POST)){                
                $arrStrJson["sucesso"]  = "true";
                $arrStrJson["mensagem"] = MensagemHelper::getInstance()->getOperacaoRealizadaComSucesso();
            }        
        }elseif($strAcao == "AlterarStatusConclusao"){
            if(FachadaLivroPrevio::getInstance()->alterarStatusConclusao($_POST)){                
                $arrStrJson["sucesso"]  = "true";
                $arrStrJson["mensagem"] = MensagemHelper::getInstance()->getOperacaoRealizadaComSucesso();
            }
        }
