<?php    
    ...
    try{
        if($strAcao == "Consultar"){            
            $arrObjs = FachadaLivroPrevio::getInstance()->consultarFolhaPrevio($_POST);
            if($arrObjs != null){
            ...
            if(FachadaLivroPrevio::getInstance()->salvarFolhaPrevio($_POST)){                
                $arrStrJson["sucesso"]  = "true";
                $arrStrJson["mensagem"] = MensagemHelper::getInstance()->getOperacaoRealizadaComSucesso();
            }
        }elseif($strAcao == "Excluir"){
            if(FachadaLivroPrevio::getInstance()->excluirFolhaPrevio($_POST)){                
                $arrStrJson["sucesso"]  = "true";
                $arrStrJson["mensagem"] = MensagemHelper::getInstance()->getOperacaoRealizadaComSucesso();
            }
        }
        ...
        elseif($strAcao == "GetPermissaoAddLinhaFolha"){
            if(FachadaLivroPrevio::getInstance()->getPermissaoAddLinhaFolha($_POST)){                    
                $arrStrJson["sucesso"]  = "true";
            }else{
                $arrStrJson["sucesso"]  = "false";
            }
