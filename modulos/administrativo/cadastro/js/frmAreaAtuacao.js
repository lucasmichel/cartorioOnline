...
var controlador  = "../../administrativo/cadastro/controladores/AreaAtuacaoControlador.php";
var dg           = $('#grid');
...
function consultar(){ 
    ...
    if(data.sucesso == "false"){
        ...
        var params = {
            ...
            AAT_Descricao: pesquisaDescricao,
            AAT_Status: pesquisaStatus,
            limit: limitConsulta,
            offset: 0
        };
...
function initGRID(){ 
    dg.datagrid({
                        ...
                        $.post(controlador, {
                            ...
                            function(data){                                
                                if(data.sucesso == "true"){                                      
                                    $("#hddID").val(data.rows[0].AAT_ID);                                      
                                    $("#txtDescricao").val(data.rows[0].AAT_Descricao);                                                                                                                
                                    if(data.rows[0].AAT_Status == "A"){
        ...
        ,mapper:[{
            name: 'AAT_ID', title: 'Cód.', width: 60, align: 'left'
        },{
            name: 'AAT_Descricao', title: 'Descrição', width: 880, align: 'left'        
        },{
            name: 'AAT_Status', title: 'Ativo', width: 60, align: 'center', render: function(d) {
                var status = 'SIM';
                if(d == "I"){
                    status = "NÃO";
                }
...
function salvar(){        
    ...
    if($.trim($('#txtDescricao').val()) == ""){                
        ...
        $("#dialog-atencao").html("Por favor, informe descrição da área de atuação.");
        $("#dialog-atencao").dialog("open");
        return;
    }
