// codificação utf-8
var dg = $('#grid');
var controlador = "../../administrativo/cadastro/controladores/EstadoCivilControlador.php";
...
function consultar(){ 
    ...
    if(data.sucesso == "false"){
        ...
        var params = {
            ...
            ECV_Descricao: pesquisaDescricao,
            ECV_Status: pesquisaStatus,
            limit: limitConsulta,
            offset: 0
        };
...
function initGRID(){ 
    dg.datagrid({
                        ...
                        $.post(controlador, {
                            ...
                            function(data){                                
                                if(data.sucesso == "true"){                                      
                                    $("#hddID").val(data.rows[0].ECV_ID);                                      
                                    $("#txtDescricao").val(data.rows[0].ECV_Descricao);
                                    
                                    if(data.rows[0].ECV_ExigeConjuge == "N"){
                                        $("#ckbConjuge").prop("checked", false);
                                    }else{
                                        $("#ckbConjuge").prop("checked", true);
                                    }
                                    
                                    if(data.rows[0].ECV_Status == "A"){
        ...
        ,mapper:[{
            name: 'ECV_ID', title: 'Cód.', width: 60, align: 'left'
        },{
            name: 'ECV_Descricao', title: 'Descrição', width: 880, align: 'left'        
        },{
            name: 'ECV_Status', title: 'Ativo', width: 60, align: 'center', render: function(d) {
                var status = 'SIM';
                if(d == "I"){
                    status = "NÃO";
                }
