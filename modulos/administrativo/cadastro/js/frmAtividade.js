...
var controlador  = "../../administrativo/cadastro/controladores/AtividadeControlador.php";
var dg           = $('#grid');
...
function consultar(){ 
    ...
    if(data.sucesso == "false"){
        ...
        var params = {
            ...
            ATV_Descricao: pesquisaDescricao,
            ATV_Status: pesquisaStatus,
            limit: limitConsulta,
            offset: 0
        };
...
function initGRID(){ 
    dg.datagrid({
                        ...
                        $.post(controlador, {
                            ...
                            function(data){                                
                                if(data.sucesso == "true"){                                    
                                    $("#txtDescricao").val(data.rows[0].ATV_Descricao);
                                    
                                    
                                    if(data.rows[0].ATV_ExigeData == "S"){ 
                                        $("#ckbExigeData").prop("checked", true);
                                    }
                                    if(data.rows[0].ATV_Status == "I"){ 
        ...
        ,mapper:[{
            name: 'ATV_ID', title: 'Cód.', width: 60, align: 'left'
        },{
            name: 'ATV_Descricao', title: 'Descrição', width: 780, align: 'left'        
        },{
            name: 'ATV_ExigeData', title: 'Exige data?', width: 110, align: 'center', render: function(d) {
                var status = 'SIM';
                if(d == "N"){
                    status = "NÃO";
                }
            ...
            name: 'ATV_Status', title: 'Ativo', width: 60, align: 'center', render: function(d) {
                var status = 'SIM';
                if(d == "I"){
                    status = "NÃO";
                }
...
function salvar(){
    ...
    if($.trim($('#txtDescricao').val()) == ""){        
        ...
        $("#dialog-atencao").html("Por favor, informe o nome da atividade.");
        $("#dialog-atencao").dialog("open");
        return;
    }
...
function cancelar(){
    ...
    $("#ckbExigeData").prop("checked", false);    
