...
var controlador  = "../../administrativo/cadastro/controladores/AreaMinisterialControlador.php";
var dg           = $('#grid');
...
function consultar(){ 
    ...
    if(data.sucesso == "false"){
        ...
        var params = {
            ...
            AMI_Descricao: pesquisaDescricao,
            AMI_Status: pesquisaStatus,
            limit: limitConsulta,
            offset: 0
        };
...
function initGRID(){ 
    dg.datagrid({
                        ...
                        $.post(controlador, {
                            ...
                            function(data){                                
                                if(data.sucesso == "true"){                                      
                                    $("#hddID").val(data.rows[0].AMI_ID);                                      
                                    $("#txtDescricao").val(data.rows[0].AMI_Descricao);                                                                                                                
                                    if(data.rows[0].AMI_Status == "A"){
        ...
        ,mapper:[{
            name: 'AMI_ID', title: 'Cód.', width: 60, align: 'left'
        },{
            name: 'AMI_Descricao', title: 'Descrição', width: 880, align: 'left'        
        },{
            name: 'AMI_Status', title: 'Ativo', width: 60, align: 'center', render: function(d) {
                var status = 'SIM';
                if(d == "I"){
                    status = "NÃO";
                }
...
function salvar(){        
    ...
    if($.trim($('#txtDescricao').val()) == ""){                
        ...
        $("#dialog-atencao").html("Por favor, informe descrição da área ministerial.");
        $("#dialog-atencao").dialog("open");
        return;
    }
...
function excluir(){
    ...
    $.post(controlador, {
        ...
        AMI_ID: dg.datagrid('getSelectedRows')[0].cells[0].innerHTML
    },
        function(data) {
            var dialog = "dialog-sucesso";
