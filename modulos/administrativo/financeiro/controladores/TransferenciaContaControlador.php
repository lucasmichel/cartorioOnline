<?php    
    ...
    try{
        if($strAcao == "Consultar"){
            $arrObjs = FachadaFinanceiro::getInstance()->consultarContaTransferencia($_POST);            
            if($arrObjs != null){
            ...
            if(FachadaFinanceiro::getInstance()->salvarContaTransferencia($_POST)){                
                $arrStrJson["sucesso"]  = "true";
                $arrStrJson["mensagem"] = MensagemHelper::getInstance()->getOperacaoRealizadaComSucesso();
            }
        }elseif($strAcao == "Excluir"){
            if(FachadaFinanceiro::getInstance()->excluirContaTransferencia($_POST)){                
                $arrStrJson["sucesso"]  = "true";
                $arrStrJson["mensagem"] = MensagemHelper::getInstance()->getOperacaoRealizadaComSucesso();
            }        
        }elseif($strAcao == "GetValorContaBancaria"){
            $arrDados = FachadaFinanceiro::getInstance()->getSaldoContaBancaria($_POST);
            if($arrDados != null){
                $arrStrJson["rows"]     = $arrDados;
                $arrStrJson["sucesso"]  = "true";
                $arrStrJson["mensagem"] = MensagemHelper::getInstance()->getOperacaoRealizadaComSucesso();
            }
        }
