<?php    
    ...
    try{
        if($strAcao == "Consultar"){            
            $arrObjs = FachadaFinanceiro::getInstance()->consultarFluxoCaixa($_POST);
            ...
            if(FachadaFinanceiro::getInstance()->salvarFluxoCaixa($_POST)){                
                $arrStrJson["sucesso"]  = "true";
                $arrStrJson["mensagem"] = MensagemHelper::getInstance()->getOperacaoRealizadaComSucesso();
            }
        }elseif($strAcao == "Excluir"){                        
            if(FachadaFinanceiro::getInstance()->excluirFluxoCaixa($_POST)){                
                $arrStrJson["sucesso"]  = "true";
                $arrStrJson["mensagem"] = MensagemHelper::getInstance()->getOperacaoRealizadaComSucesso();
            }
