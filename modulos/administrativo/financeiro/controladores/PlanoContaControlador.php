<?php    
    ...
    try{
        if($strAcao == "Consultar"){
            $arrObjs = FachadaFinanceiro::getInstance()->consultarPlanoConta($_POST);
            ...
            if(FachadaFinanceiro::getInstance()->salvarPlanoConta($_POST)){                
                $arrStrJson["sucesso"]  = "true";
                $arrStrJson["mensagem"] = MensagemHelper::getInstance()->getOperacaoRealizadaComSucesso();
            }
        }elseif($strAcao == "ConsultarCodigoContabil"){            
            $arrObjs = FachadaFinanceiro::getInstance()->consultarPlanoConta($_POST);
            if($arrObjs == null){    
                $arrStrJson["rows"]     = $arrObjs["rows"];
                $arrStrJson["num_rows"] = $arrObjs["num_rows"];
            ...
            if(FachadaFinanceiro::getInstance()->excluirPlanoConta($_POST)){                
                $arrStrJson["sucesso"]  = "true";
                $arrStrJson["mensagem"] = MensagemHelper::getInstance()->getOperacaoRealizadaComSucesso();
            }
        }         
