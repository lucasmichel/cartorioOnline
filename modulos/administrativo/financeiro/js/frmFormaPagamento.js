...
var controlador  = "../../administrativo/financeiro/controladores/FormaPagamentoControlador.php";
var dg           = $('#grid');
    ...
    if(data.sucesso == "false"){
        ...
        var params = {
            ACO_Descricao: "Consultar", 
                FPG_Descricao: pesquisaDescricao,
                FPG_Status: pesquisaStatus,
                limit: limitConsulta,
                offset: 0
        };
...
function initGRID(){ 
    ...
    dg.datagrid({
        ...
        ,toolBarButtons: [
            {
                ...
                ,fn: function() {
                    if(dg.datagrid('getSelectedRows').length > 0){
                        ...
                        // ao controlador qual registro ser� modificado
                        ...
                        $.post(controlador, {
                            ...
                            function(data){                                
                                if(data.sucesso == "true"){                                      
                                    $("#hddID").val(data.rows[0].FPG_ID);                                      
                                    $("#txtDescricao").val(data.rows[0].FPG_Descricao);                                        
                                    if(data.rows[0].FPG_Status == "A"){
                                        $("#ckbStatus").prop("checked", false);
                                    }else{
                                        $("#ckbStatus").prop("checked", true);
                                    }
                                    if(data.rows[0].FPG_ExigeNumero == "N"){
                                        $("#ckbExigeNumero").prop("checked", false);
                                    }else{
                                        $("#ckbExigeNumero").prop("checked", true);
                                    }
        ...
        ,mapper:[{
            name: 'FPG_ID', title: 'C&oacute;d.', width: 60, align: 'left'
        },{
            name: 'FPG_Descricao', title: 'Descri&ccedil;&atilde;o', width: 720, align: 'left'        
        },{
            name: 'FPG_ExigeNumero', title: 'Exige Número', width: 120, align: 'center', render: function(d) {
                var exigir = 'SIM';
                if(d == "N"){
                    exigir = "N&Atilde;O";
                }
            ...
            name: 'FPG_Status', title: 'Ativo', width: 60, align: 'center', render: function(d) {
                var status = 'SIM';
                if(d == "I"){
                    status = "N&Atilde;O";
                }
...
function salvar(){        
    ...
    // o sistema entender� que a a��o que ser� executada 
    // dever� ser o ALTERAR
    if($.trim($("#hddID").val()) != ""){
    ...
    if($.trim($('#txtDescricao').val()) == ""){                
        ...
        $("#dialog-atencao").html("Por favor, informe descri&ccedil;&atilde;o.");
        $("#dialog-atencao").dialog("open");
