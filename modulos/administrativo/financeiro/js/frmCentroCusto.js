...
var controlador  = "../../administrativo/financeiro/controladores/CentroCustoControlador.php";
var dg           = $('#grid');
    ...
    if(data.sucesso == "false"){
        ...
        var params = {
            ACO_Descricao: "Consultar", 
                CEN_Descricao: pesquisaDescricao,
                CEN_Status: pesquisaStatus,
                limit: limitConsulta,
                offset: 0
        };
...
function initGRID(){ 
    dg.datagrid({
        ...
        ,toolBarButtons: [
            {
                        ...
                        $.post(controlador, {
                            ...
                            function(data){                                
                                if(data.sucesso == "true"){                                      
                                    $("#hddID").val(data.rows[0].CEN_ID);                                      
                                    $("#txtDescricao").val(data.rows[0].CEN_Descricao);    
                                    $("#txtAnotacao").val(data.rows[0].CEN_Observacao);                                        
                                    if(data.rows[0].CEN_Status == "A"){
                ...
                ,fn: function(){
                    if(dg.datagrid('getSelectedRows').length > 0){                        
                        $("#dialogMensagemExcluir").html("Tem certeza que deseja remover registro selecionado?");
        ...
        ,mapper:[{
            name: 'CEN_ID', title: 'Cód.', width: 60, align: 'left'
        },{
            name: 'CEN_Descricao', title: 'Descrição', width: 280, align: 'left'        
        },{
            name: 'CEN_Observacao', title: 'Anotação', width: 580, align: 'left'                
        },{
            name: 'CEN_Status', title: 'Ativo', width: 60, align: 'center', render: function(d) {
                var status = 'SIM';
                if(d == "I"){
                    status = "NÃO";
                }
...
function salvar(){        
    ...
    if($.trim($('#txtDescricao').val()) == ""){                
        ...
        $("#dialog-atencao").html("Por favor, informe descrição do centro de custo.");
        $("#dialog-atencao").dialog("open");
