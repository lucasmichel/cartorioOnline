...
var controlador  = "../../administrativo/patrimonio/controladores/FormaAquisicaoControlador.php";
var dg           = $('#grid');
...
function consultar(){ 
    ...
    if(data.sucesso == "false"){
        ...
        var params = {
            ...
            FRA_Descricao: pesquisaDescricao,
            FRA_Status: pesquisaStatus,
            limit: limitConsulta,
            offset: 0
        };
...
function initGRID(){ 
    dg.datagrid({
                        ...
                        $.post(controlador, {
                            ...
                            function(data){                                
                                ...
                                if(data.sucesso == "true"){
                                    $("#hddID").val(data.rows[0].FRA_ID);
                                    $("#txtDescricao").val(data.rows[0].FRA_Descricao);

                                    if(data.rows[0].FRA_Status == "I") $("#ckbStatus").prop("checked", true);
        ...
        ,mapper:[{
            name: 'FRA_ID', title: 'Cód.', width: 60, align: 'center'
        },{
            name: 'FRA_Descricao', title: 'Descrição', width: 880, align: 'left'        
        },{
            name: 'FRA_Status', title: 'Ativo', width: 60, align: 'center', render: function(d) {
                var status = 'SIM';
                if(d == "I"){
                    status = "NÃO";
                }
