<?php    
    ...
    try{
        if($strAcao == "Consultar"){
            $arrObjs = FachadaPatrimonio::getInstance()->consultarTipoPatrimonio($_POST);
            ...
            if(FachadaPatrimonio::getInstance()->salvarTipoPatrimonio($_POST)){                
                $arrStrJson["sucesso"]  = "true";
                $arrStrJson["mensagem"] = MensagemHelper::getInstance()->getOperacaoRealizadaComSucesso();
            }
        }elseif($strAcao == "Excluir"){             
            if(FachadaPatrimonio::getInstance()->excluirTipoPatrimonio($_POST)){                
                $arrStrJson["sucesso"]  = "true";
                $arrStrJson["mensagem"] = MensagemHelper::getInstance()->getOperacaoRealizadaComSucesso();
            }
        }
