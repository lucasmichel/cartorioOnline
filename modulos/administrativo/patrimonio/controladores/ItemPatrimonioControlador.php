<?php    
    ...
    try{
        if($strAcao == "Consultar"){            
            $arrObjs = FachadaPatrimonio::getInstance()->consultarItemPatrimonio($_POST);            
            if($arrObjs != null){
            ...
            if(FachadaPatrimonio::getInstance()->salvarItemPatrimonio($_POST)){                
                $arrStrJson["sucesso"]  = "true";
                $arrStrJson["mensagem"] = MensagemHelper::getInstance()->getOperacaoRealizadaComSucesso();
            }
        }elseif($strAcao == "Excluir"){              
            if(FachadaPatrimonio::getInstance()->excluirItemPatrimonio($_POST)){                
                $arrStrJson["sucesso"]  = "true";
                $arrStrJson["mensagem"] = MensagemHelper::getInstance()->getOperacaoRealizadaComSucesso();
            }
        }
