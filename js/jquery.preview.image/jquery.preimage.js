(function( $ ){
     	    ...
     	    for(i=0; i<this.files.length; i++){
	    		...
	    		reader.onload = function (e) {
	    			$('<div />').css({'background-image': ('url('+e.target.result+')'), 'background-repeat': 'no-repeat', 'background-size': settings['scale'] }).addClass(settings['prefix']+'thumb').appendTo($('#'+settings['prefix']+id));
                                //setei o valor da escala para 170px... se quiser alterar descomentar a linha acima e comentar a de baixo
	    			//$('<div />').css({'background-image': ('url('+e.target.result+')'), 'background-repeat': 'no-repeat', 'background-size': '170px 170px'}).addClass(settings['prefix']+'thumb').appendTo($('#'+settings['prefix']+id));
	    		};
  ...
  $.fn.preimage = function( method ) {
    ...
    } else if (typeof method === 'destroy') {
		//$.error( 'Method ' +  method + ' does not exist on jQuery.preimage' );                
                return methods.destroy.apply();
    }    
